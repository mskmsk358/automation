...
test('Secure2 Forgot Password Page Validation', async ({
  browser,
  baseURL,
}) => {
  ...
  const forgotPasswordText =
    await forgotpasswordpage.ForgotPasswordText.textContent();
  expect(forgotPasswordText).toContain('Forgot password');

  const forgotpasswordConfirmText =
    await forgotusernamepage.IndForgotUserNameConfirmText.textContent();
  expect(forgotpasswordConfirmText).toContain(
    ' Please provide the information below to recover your password ',
  );
  ...
  const repassworderror = await page
    .locator('mat-error#pwdReEnterInvalidErr>p')
    .textContent();
  ...
  const usedpassworderror = await page
    .locator('#notificationMessage')
    .textContent();
  expect(usedpassworderror).toContain(
    'Passwords used within the past 12 months cannot be used. Please create a new password.',
  );
