...
test('Secure2 Forgot Username Page Validation', async ({ browser, baseURL }) => {
    ...
    const forgotusernameText = await (forgotusernamepage.IndForgotUserNameText).textContent();
    expect(forgotusernameText).toContain("Forgot username");

    const forgotusernameConfirmText = await (forgotusernamepage.IndForgotUserNameConfirmText).textContent();
    expect(forgotusernameConfirmText).toContain(" Please provide the information below to recover your username. ");
    ...
    expect(DobError2).toContain(" Please re-enter date of birth, you cannot be over 110 years old.");
   ...
   /* await page.keyboard.press('Backspace');
    ...
    const sucessMessage = await page.locator("##notificationMessage").textContent();
    expect(sucessMessage).toContain("An email with your username has been sent to");
